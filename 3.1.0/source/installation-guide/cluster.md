...
1. [Install Gluu](https://gluu.org/docs/ce/3.1.0/installation-guide/install/) on one server making sure to use a separate NGINX server FQDN as hostname.
    ...
    - A separate NGINX server is recommended, but not necessary, 
      since replicating a Gluu server to a different hostname breaks the functionality 
      of the Gluu webpage, when using a hostname other than what is in the certificates. 
      For example, if I used c1.gluu.info as my host and another install of gluu as `c2.gluu.info`, 
      the process of accessing the site on `c2.gluu.info`, even with replication, will fail 
      authentication. So if c1 failed, you could not access the Gluu web GUI anymore.
...
2. Copy the Gluu install environment to the other servers. 
    ...
    - Now tar the `/opt/gluu-server-3.1.0`/ folder, copy it to the other servers and 
    extract it in the /opt/ folder.
...
3. Start Gluu, login and modify the `/etc/hosts/` inside the chroot to point the FQDN of the NGINX server to the current servers IP address
    ...
    - This is necessary to deal with internal routing of NGINX to Apache2 and Apache2 to NGINX. 
    So even though my ip of my FQDN is different, this process still works.

4. There needs to be primary server to replicate from initially for delta-syncrepl to inject data from. 
After the initial sync, all servers will be exactly the same, as delta-syncrepl will fill the newly created 
database. 
...
5. Now is where we will set servers to associate with each other for MMR by editing the slapd.conf, 
ldap.conf and symas-openldap.conf files.

    - Creating the slapd.conf file is relatively easy, but can be prone to errors if done manually. 
    Attached is a script and template files for creating multiple slapd.conf files for every server. 
    Download git and clone the necessary files:
    ...
    - Here we want to change the `ip_address`, `fqn_hostname`, `ldap_password` to our specific server instances. 
    For example:
    ...
    - Move each .conf file to their respective server @:
            ...
            `/opt/gluu-server-3.1.0/opt/symas/etc/openldap/slapd.conf`
...
9. Now let's configure your NGINX server for oxTrust and oxAuth web failover. 
    ...
    - Put the following template in it's place. Make sure to change the `{serverX_ip_or_FQDN}` portion to 
    your servers IP addresses or FQDN under the upstream section. Add as many servers as exist in your 
    replication setup. The `server_name` needs to be your NGINX servers FQDN.    
    ...
    - Now all traffic for the Gluu web GUI will route through one address i.e. `nginx.gluu.info`. 
    This gives us fail-over redundancy for our Gluu web GUI if any server goes down, as NGINX automatically 
    does passive health checks.   
