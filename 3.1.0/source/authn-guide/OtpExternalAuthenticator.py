...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, configurationAttributes):
        ...
        if not configurationAttributes.containsKey("otp_type"):
            print
            "OTP. Initialization. Property otp_type is mandatory"
            return False
        self.otpType = configurationAttributes.get("otp_type").getValue2()

        if not self.otpType in ["hotp", "totp"]:
            print
            "OTP. Initialization. Property value otp_type is invalid"
            return False

        if not configurationAttributes.containsKey("issuer"):
            print
            "OTP. Initialization. Property issuer is mandatory"
            return False
        ...
        if step == 1:
            ...
            if otp_auth_method == "authenticate":
                ...
                if len(user_enrollments) == 0:
                    otp_auth_method = "enroll"
                    print
                    "OTP. Authenticate for step 1. There is no OTP enrollment for user '%s'. Changing otp_auth_method to '%s'" % (
                    user_name, otp_auth_method)

            if otp_auth_method == "enroll":
                print
                "OTP. Authenticate for step 1. Setting count steps: '%s'" % 3
                context.set("otp_count_login_steps", 3)

            print
            "OTP. Authenticate for step 1. otp_auth_method: '%s'" % otp_auth_method
            context.set("otp_auth_method", otp_auth_method)

            return True
        elif step == 2:
            print
            "OTP. Authenticate for step 2"
            ...
            if otp_auth_method == 'enroll':
                ...
                if not StringHelper.isEmpty(auth_result):
                    print
                    "OTP. Authenticate for step 2. User not enrolled OTP"
                    return False

                print
                "OTP. Authenticate for step 2. Skipping this step during enrollment"
                return True

            otp_auth_result = self.processOtpAuthentication(requestParameters, user_name, session_attributes,
                                                            otp_auth_method)
            print
            "OTP. Authenticate for step 2. OTP authentication result: '%s'" % otp_auth_result

            return otp_auth_result
        elif step == 3:
            print
            "OTP. Authenticate for step 3"
            ...
            otp_auth_result = self.processOtpAuthentication(requestParameters, user_name, session_attributes,
                                                            otp_auth_method)
            print
            "OTP. Authenticate for step 3. OTP authentication result: '%s'" % otp_auth_result
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if step == 1:
            print
            "OTP. Prepare for step 1"

            return True
        elif step == 2:
            print
            "OTP. Prepare for step 2"
            ...
            otp_auth_method = session_attributes.get("otp_auth_method")
            print
            "OTP. Prepare for step 2. otp_auth_method: '%s'" % otp_auth_method
            ...
            if otp_auth_method == 'enroll':
                ...
                if self.otpType == "hotp":
                    ...
                    otp_enrollment_request = self.generateTotpSecretKeyUri(otp_secret_key, self.otpIssuer,
                                                                           user.getAttribute("displayName"))
                else:
                    print
                    "OTP. Prepare for step 2. Unknown OTP type: '%s'" % self.otpType
                    return False

                print
                "OTP. Prepare for step 2. Prepared enrollment request for user: '%s'" % user.getUserId()
            ...
            otp_auth_method = session_attributes.get("otp_auth_method")
            print
            "OTP. Prepare for step 3. otp_auth_method: '%s'" % otp_auth_method
    ...
    def getPageForStep(self, configurationAttributes, step):
        if step == 2:
            ...
            otp_auth_method = session_attributes.get("otp_auth_method")
            print
            "OTP. Gep page for step 2. otp_auth_method: '%s'" % otp_auth_method
    ...
    def loadOtpConfiguration(self, configurationAttributes):
        ...
        try:
            ...
            print
            "OTP. Load OTP configuration. Failed to load configuration from file:", otp_conf_file
            return False
        finally:
            f.close()
        ...
        try:
            ...
            if StringHelper.equalsIgnoreCase(hmacShaAlgorithm, "sha1"):
                ...
                print
                "OTP. Load OTP configuration. Invalid TOTP HMAC SHA algorithm: '%s'" % hmacShaAlgorithm

            self.totpConfiguration["hmacShaAlgorithmType"] = hmacShaAlgorithmType
        except:
            print
            "OTP. Load OTP configuration. Invalid configuration file '%s' format. Exception: '%s'" % (
    ...
    def processBasicAuthentication(self, credentials):
        ...
        if find_user_by_uid == None:
            print
            "OTP. Process basic authentication. Failed to find user '%s'" % user_name
            return None
    ...
    def findEnrollments(self, user_name, skipPrefix=True):
        ...
        if user == None:
            print
            "OTP. Find enrollments. Failed to find user"
            return result
    ...
    def validateSessionState(self, session_attributes):
        ...
        if StringHelper.isEmpty(session_state):
            print
            "OTP. Validate session state. Failed to determine session_state"
            return False

        otp_auth_method = session_attributes.get("otp_auth_method")
        if not otp_auth_method in ['enroll', 'authenticate']:
            print
            "OTP. Validate session state. Failed to authenticate user. otp_auth_method: '%s'" % otp_auth_method
            return False
    ...
    def processOtpAuthentication(self, requestParameters, user_name, session_attributes, otp_auth_method):
        ...
        if StringHelper.isEmpty(otpCode):
            ...
            print
            "OTP. Process OTP authentication. otpCode is empty"
        ...
        if otp_auth_method == "enroll":
            ...
            if otp_secret_key_encoded == None:
                print
                "OTP. Process OTP authentication. OTP secret key is invalid"
                return False
            ...
            if self.otpType == "hotp":
                ...
                if (validation_result != None) and validation_result["result"]:
                    ...
                    find_user_by_external_uid = userService.addUserAttribute(user_name, "oxExternalUid",
                                                                             otp_user_external_uid)
                    if find_user_by_external_uid != None:
                        return True

                    print
                    "OTP. Process HOTP authentication during enrollment. Failed to update user entry"
            elif self.otpType == "totp":
                validation_result = self.validateTotpKey(otp_secret_key, otpCode)
                if (validation_result != None) and validation_result["result"]:
                    print
                    "OTP. Process TOTP authentication during enrollment. otpCode is valid"
                    ...
                    print
                    "OTP. Process TOTP authentication during enrollment. Failed to update user entry"
        elif otp_auth_method == "authenticate":
            user_enrollments = self.findEnrollments(user_name)

            if len(user_enrollments) == 0:
                print
                "OTP. Process OTP authentication. There is no OTP enrollment for user '%s'" % user_name
            ...
            if self.otpType == "hotp":
                for user_enrollment in user_enrollments:
                    ...
                    if (validation_result != None) and validation_result["result"]:
                        print
                        "OTP. Process HOTP authentication during authentication. otpCode is valid"
                        ...
                        find_user_by_external_uid = userService.replaceUserAttribute(user_name, "oxExternalUid",
                                                                                     otp_user_external_uid,
                        ...
                        print
                        "OTP. Process HOTP authentication during authentication. Failed to update user entry"
            elif self.otpType == "totp":
                for user_enrollment in user_enrollments:
                    ...
                    if (validation_result != None) and validation_result["result"]:
                        print
                        "OTP. Process TOTP authentication during authentication. otpCode is valid"
                        return True
    ...
    def generateTotpKey(self, secretKey):
        ...
        totp = TOTP.key(secretKey).digits(digits).timeStep(TimeUnit.SECONDS.toMillis(timeStep)).hmacSha(
            hmacShaAlgorithmType).build()
    ...
    def generateTotpSecretKeyUri(self, secretKey, issuer, userDisplayName):
        ...
        otpAuthURI = OTPAuthURIBuilder.fromKey(otpKey).label(label).issuer(issuer).digits(digits).timeStep(
            TimeUnit.SECONDS.toMillis(timeStep)).build();
