...
| Install Gluu Server     | `# apt-get install gluu-server-3.1.7`      |
...
| Install Gluu Server     | `# yum install gluu-server-3.1.7`          |
...
```
# /sbin/gluu-serverd-3.1.7 enable
# /sbin/gluu-serverd-3.1.7 start
...
```
# service gluu-server-3.1.7 stop
# apt-get remove gluu-server-3.1.7
# rm -rf /opt/gluu-server-3.1.7.save
```
...
```
# /sbin/gluu-serverd-3.1.7 disable
# /sbin/gluu-serverd-3.1.7 stop
# yum remove gluu-server-3.1.7 
# rm -rf /opt/gluu-server-3.1.7.save
```

!!! Note
    `apt-get purge gluu-server-3.1.7` or `apt-get remove --purge gluu-server-3.1.7` can also be used to uninstall and remove all the folders and services of the Gluu Server. Make sure to backup ALL directories of `/opt` into other direction ( tmp or root directory itself ) before running purge command. 
