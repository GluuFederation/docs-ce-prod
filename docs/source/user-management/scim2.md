...
In addition, more details about Gluu's implementation is available in the [SCIM API docs](../api-guide/scim-api.md).

## API Protection
The SCIM API should never be accessed anonymously. However, the standard itself doesn't define a mechanism to prevent unauthorized requests to endpoints. There are just a few guidelines for authentication and authorization in section 2 of [RFC 7644](https://tools.ietf.org/html/rfc7644). 
