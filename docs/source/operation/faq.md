...
```
$ tail -f /opt/gluu-server-3.1.5/opt/gluu/jetty/identity/logs/* \
          /opt/gluu-server-3.1.5/opt/gluu/jetty/oxauth/logs/*
```       
...
```
# /etc/init.d/gluu-server-3.1.5 stop
# rm -rf /opt/gluu-server-3.1.5/opt/jetty-9.3/temp/*
# /etc/init.d/gluu-server-3.1.5 start
```
...
```
# /etc/init.d/gluu-server-3.1.5 login
...
You can find the configuration you need in `/opt/gluu-server-3.1.5/etc/gluu/conf/ox-ldap.properties`, e.g.:
