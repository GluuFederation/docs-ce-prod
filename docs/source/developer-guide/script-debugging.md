...
The following instructions assume Gluu Server 3.1.7 is already installed and available. If not, perform a standard [Gluu Server installation](../installation-guide/index.md), then do the following: 

1. Install `https://repo.gluu.org/tools/tools-install.sh`
1. Log out of CE
1. Run `/opt/gluu-server-3.1.7/opt/gluu/bin/prepare-dev-tools.py`
1. Log in to CE
...
Running `/opt/gluu-server-3.1.7/opt/gluu/bin/prepare-dev-tools.py` allows Eclipse to access X server. 
...
cp -f /root/.Xauthority-gluu /opt/gluu-server-3.1.7/root/.Xauthority
