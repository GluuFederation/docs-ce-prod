...
Pure Python libraries can be added to `/opt/tomcat/conf/python`, and jar files can be added to  
...
When the scripting is done, you can test the script by printing the statments to `oxtrust.log` under `/opt/tomcat/logs/`. 
...
Further logs to debug and monitor the sequence can be done using `oxauth_script.log` and `oxauth.log` under `/opt/tomcat/logs/` which is within the chroot.
