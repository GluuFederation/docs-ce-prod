...
1. In `type` choose "openidconnect" (if you are using Gluu oxd as a mediator with an OP check [??](#using-oxd-as-mediator))
...
**Note:**
A simple standard [attribute mapping](??#attribute-mapping-and-transformation) is used for OpenID Connect providers by default. This will populate LDAP attributes `uid`, `mail`, `cn`, `displayName`, `givenName`, and `sn` if the relevant corresponding claims were gathered in the `userInfo` request.
...
The procedure for registering a client at the external provider may vary. In some setups, the OP may support dynamic registration of clients. Check the product documentation and inquire the admin of the provider to integrate.
...
The same steps described for [Social Login](#Protect the application with `passport_social` authentication) can be followed in this case.

If additional assistance is needed, open a ticket on [Gluu support](https://support.gluu.org).

### Extra topics

#### Using oxd as mediator

When using oxd, administrators can follow the steps similar as [above](#integrating-openid-connect-providers) taking in the account these considerations:

- For provider `type` select "openidconnect-oxd"
- Instead of creating a client directly, a call to OXD Server [`register-site` API method](https://gluu.org/docs/oxd/4.0.beta/api/#register-site) must be issued. [Here](https://github.com/GluuFederation/passport-oxd#create-a-client) is an example. From this action, the so-called `oxdID` is obtained 
...
1. To [register a client](#register-a-client-at-the-op) in your remote Gluu server. Login with admin credentials to `https://<remote-gluu-server>/identity` and navigate to `OpenID` > `Clients` > `Add`. Provide the following settings:
...
The following are the steps required to offer social login for an OIDC application:
...
1. For Passport.js strategy use the reference table below:
    ...
    If the provider of interest is not listed, it is necessary to find a proper [node package](https://www.npmjs.com/) for that provider and installing it. Recall the package **has to be** actually a Passport.js strategy based on OAuth 1.0 or OAuth 2.0.
...
1. Fill the name of the applicable mapping. Use the following table as reference:
    ...
    It is recommended to create mappings based on existing mapping files. Make a copy of any files listed in the table above (see directory `/opt/gluu/node/passport/server/mappings` in Gluu chroot) and name it appropriately. Enter the name (without file extension) in this field. The [tutorial](??) contains instructions on how to write attribute mappings. It is an easy task and generally does not demand programming skills.
    
1. If the provider being added is present in the table above, you can leave the `logo path` field blank. Otherwise check this [section](??#about-logo-images) of the introductory page.
...
1. `Authenticate params` is a field that normally can be left empty. It is used to supply the value for the second parameter of Passport.js method `passport.authenticate`. It is recommended to supply data here only when the provider to be added is not listed in the table above.
    ...
    As an example suppose [VKontakte](http://www.vk.com) is the external provider to integrate. To have access to user email a proper scope must be specified (check [here](https://github.com/stevebest/passport-vkontakte#extended-permissions) and [here](https://vk.com/dev/permissions)). In this case the field `Authenticate params` can be filled this way: `{ scope: ['email'] }`.
    
    !!! Warning:
    Only static values will work. No Javascript dynamic expressions should be included. For instance `{ key: Math.random() }` may provoke Node runtime to crash.
...
1. Checking `Request For Email` or `Email linking` is not needed unless you want to exercise a [custom flow](??#altering-flow-behaviour).

1. Leave the fields under the `Providers Options` empty and click on `Add`.

### Obtain client credentials

Every provider has its own procedure for issuing client credentials (ie. client ID and client secret). Check the developer docs of the specific social site for more information. The aim is to get to a page that allows creation of applications. Here are links for a few popular providers: 
...
As an example, for VKontakte the [docs](https://github.com/stevebest/passport-vkontakte#profile-fields) state strategy instantiation can have the following form in order to request email, city, and birth date in addition to typical profile fields:
...
If this behavior is desired, click on `Add new property`, fill on the left with `profileFields` and on the right with the actual value for this property (eg. `['email', 'city', 'bdate']`). 
...
- Some strategies do not use the property `clientID` but `consumerKey`.  This property is automatically generated. The same applies for `clientSecret`/`consumerSecret`.
- If neither `clientID` nor `consumerKey` is used, but a different name, you have to supply that as a separate property (click on the `Add new property button`). This is the case for example of [wechat](https://github.com/liangyali/passport-wechat#configure--strategy) where `appID` and `appSecret` are used instead.
...
If you get an error page like the one below, double check your configuration and Internet access from both your browser and VM. Also check the [logs](??#files-and-severity-level) contents.
...
To check the actual profile data received during the authentication transaction, review the [logs](??#files-and-severity-level) and search for a message that looks like "Resulting profile data is". To be able to view this message, set logging level to debug. And wait for the server to pick the changes.  
