...
## Directory Service
The Gluu Server uses the open source OpenLDAP server as its internal directory server to store data generated by the authentication and authorization service, such as user profiles, configuration data, tokens and credentials. 

## Identity Management
The Gluu Server does **not** do things like delegated administration, role definition, approvals and workflows, etc. 

However, via the Gluu Server interface and the OpenLDAP directory, admins can manage user data generated by the authentication service. If you have an existing identity management system, the Gluu Server can use it as the authoritative source of identity information using the System for Cross-domain Identity Management protocol, aka [SCIM]. 

Learn more in the [user management guide](./user-management/local-user-management.md)
