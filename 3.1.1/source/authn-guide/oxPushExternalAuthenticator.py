...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, configurationAttributes):
        ...
        print
        "oxPush. Initialized successfully"
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        if (step == 1):
            print
            "oxPush. Authenticate for step 1"
            ...
            if (find_user_by_uid == None):
                print
                "oxPush. Authenticate for step 1. Failed to find user"
                return False
            ...
            if ((user_external_uid_attr == None) or (user_external_uid_attr.getValues() == None)):
                print
                "oxPush. Authenticate for step 1. There is no external UIDs for user: ", user_name
            else:
                ...
                if (oxpush_user_uid == None):
                    print
                    "oxPush. Authenticate for step 1. There is no oxPush UID for user: ", user_name
                else:
                    # Check deployment status
                    print
                    "oxPush. Authenticate for step 1. oxpush_user_uid: ", oxpush_user_uid
                    deployment_status = self.oxPushClient.getDeploymentStatus(oxpush_user_uid);
                    if (deployment_status.result):
                        print
                        "oxPush. Authenticate for step 1. Deployment status is valid"
                        if ("enabled" == deployment_status.status):
                            print
                            "oxPush. Authenticate for step 1. Deployment is enabled"
                            context.set("oxpush_user_uid", oxpush_user_uid)
                        else:
                            print
                            "oxPush. Authenticate for step 1. Deployment is disabled"
                            return False
                    else:
                        print
                        "oxPush. Authenticate for step 1. Deployment status is invalid. Force user to pair again"
                        # Remove oxpush_user_uid from user entry
                        find_user_by_uid = userService.removeUserAttribute(user_name, "oxExternalUid",
                                                                           "oxpush:" + oxpush_user_uid)
                        if (find_user_by_uid == None):
                            print
                            "oxPush. Authenticate for step 1. Failed to update current user"
                            return False

            return True
        elif (step == 2):
            print
            "oxPush. Authenticate for step 2"
            ...
            if (sessionAttributes == None) or not sessionAttributes.containsKey("oxpush_user_uid"):
                print
                "oxPush. Authenticate for step 2. oxpush_user_uid is empty"

                if (not sessionAttributes.containsKey("oxpush_pairing_uid")):
                    print
                    "oxPush. Authenticate for step 2. oxpush_pairing_uid is empty"
                    return False
                ...
                if (pairing_status == None):
                    print
                    "oxPush. Authenticate for step 2. The pairing has not been authorized by user"
                    return False

                oxpush_user_uid = pairing_status.deploymentId

                print
                "oxPush. Authenticate for step 2. Storing oxpush_user_uid in user entry", oxpush_user_uid
                ...
                if (find_user_by_uid == None):
                    print
                    "oxPush. Authenticate for step 2. Failed to update current user"
                    return False
                ...
                print
                "oxPush. Authenticate for step 2. Deployment status is valid"

            return True
        elif (step == 3):
            print
            "oxPush. Authenticate for step 3"
            ...
            if (sessionAttributes == None) or not sessionAttributes.containsKey("oxpush_user_uid"):
                print
                "oxPush. Authenticate for step 3. oxpush_user_uid is empty"
                return False
            ...
            try:
                ...
                print
                "oxPush. Authenticate for step 3. Failed to initialize authentication process: ", err
                return False

            if (not authentication_request.result):
                print
                "oxPush. Authenticate for step 3. Failed to initialize authentication process"
                return False

            # Check authentication status
            authentication_status = self.checkStatus("authenticate", authentication_request.authenticationId,
                                                     oxpush_user_timeout)
            if (authentication_status == None):
                print
                "oxPush. Authenticate for step 3. The authentication has not been authorized by user"
                return False

            print
            "oxPush. Authenticate for step 3. The request was granted"
        ...
        try:
            ...
            while (endTime >= curTime):
                ...
                if (not response_status.result):
                    print
                    "oxPush. CheckStatus. Get false result from oxPushServer"
                    return None
                ...
                if ("declined" == status):
                    print
                    "oxPush. CheckStatus. The process has been cancelled"
                    return None

                if ("expired" == status):
                    print
                    "oxPush. CheckStatus. The process has been expired"
                    return None

                if ("approved" == status):
                    print
                    "oxPush. CheckStatus. The process was approved"
            ...
            print
            "oxPush. CheckStatus. Could not check process status: ", err
            return None

        print
        "oxPush. CheckStatus. The process has not received a response from the phone yet"
