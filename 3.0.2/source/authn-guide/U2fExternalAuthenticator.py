...
class PersonAuthentication(PersonAuthenticationType):
    ...
    def init(self, configurationAttributes):
        ...
        print
        "U2F. Initialization. Downloading U2F metadata"
        ...
        metaDataConfigurationService = Component.getInstance(FidoU2fClientFactory).createMetaDataConfigurationService(
    ...
    def authenticate(self, configurationAttributes, requestParameters, step):
        ...
        if (step == 1):
            ...
            if (user == None):
                print
                "U2F. Prepare for step 2. Failed to determine user name"
                return False

            if (auth_method == 'authenticate'):
                print
                "U2F. Prepare for step 2. Call FIDO U2F in order to finish authentication workflow"
                authenticationRequestService = Component.getInstance(
                    FidoU2fClientFactory).createAuthenticationRequestService(self.metaDataConfiguration)
                ...
                if (authenticationStatus.getStatus() != Constants.RESULT_SUCCESS):
                    print
                    "U2F. Authenticate for step 2. Get invalid authentication status from FIDO U2F server"
                    return False

                return True
            elif (auth_method == 'enroll'):
                print
                "U2F. Prepare for step 2. Call FIDO U2F in order to finish registration workflow"
                registrationRequestService = Component.getInstance(
                    FidoU2fClientFactory).createRegistrationRequestService(self.metaDataConfiguration)
                ...
                if (registrationStatus.getStatus() != Constants.RESULT_SUCCESS):
                    print
                    "U2F. Authenticate for step 2. Get invalid registration status from FIDO U2F server"
                    return False

                return True
            else:
                print
                "U2F. Prepare for step 2. Authenticatiod method is invalid"
                return False
    ...
    def prepareForStep(self, configurationAttributes, requestParameters, step):
        ...
        if (step == 1):
            ...
            if (deviceRegistrations.size() > 0):
                print
                "U2F. Prepare for step 2. Call FIDO U2F in order to start authentication workflow"
                ...
                try:
                    ...
                    if (ex.getResponse().getResponseStatus() != Response.Status.NOT_FOUND):
                        print
                        "U2F. Prepare for step 2. Failed to start authentication workflow. Exception:", sys.exc_info()[
                            1]
                        return False
            else:
                print
                "U2F. Prepare for step 2. Call FIDO U2F in order to start registration workflow"
                registrationRequestService = Component.getInstance(
                    FidoU2fClientFactory).createRegistrationRequestService(self.metaDataConfiguration)
