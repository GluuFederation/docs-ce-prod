...
- **Gluu OpenDJ/OpenLDAP***: a directory server where all Gluu user data, session data, etc. is stored.
- **Shibboleth**: a robust SAML Identity Provider (IDP)
...
## Directory Service
All data used and generated by the Gluu Server is stored in the local Gluu LDAP deployed during the installation. The LDAP includes complete details about OpenID Connect and UMA clients, user objects, and more. Learn more in the [user management guide](./user-management/local-user-management.md).
