...
## Directory Service
All data generated by the Gluu Server is stored in the local Gluu LDAP that is included during installation. In the LDAP you can see full details like how OpenID Connect and UMA clients are stored and how user objects are mapped in the LDAP tree. Learn more in the [user management guide](./user-management/local-user-management.md)

If you have an existing Active Directory or backend LDAP server, you can sync data to your local Gluu LDAP using the [Cache Refresh process](./user-management/ldap-sync.md). 
