...
We've streamlined and added features to oxTrust, including moving buttons and fields in the UI to be more aesthetically and functionally laid out, as well as adding auto-generated client secrets. We've improved our password reset flow and added support for custom attributes while importing users.

### oxAuth enhancements
We've added features and improvements to the heart of the Gluu Server, with improved client JKS expiration checks,persist client authorizations, UI and script fixes in SAML, Super Gluu, U2F and other scripts, authentication protection against brute force attempts, support for LDAP caching, and improved LDAP connection pool check.

### Enriched logging
We've updated our logs to be clearer and more useful, including logging daily instead of hourly, decreasing unnecessary logs, and making log levels a configurable parameter.
...
### New oxShibboleth IDP3 3.3.3 extension
oxAuth using Shibboleth 3.3.* now works as an authentication flow.
